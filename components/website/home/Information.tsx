  ...
  return (
    <section className="min-h-[42vh] bg-white p-3 md:p-6 gap-y-3 md:gap-y-10 flex flex-col justify-center items-center">
      <div className="max-w-[1000px]">
        <p className="text-sm md:text-2xl font-nunito leading-normal tracking-wide font-light text-center">
          {information?.content || ""}
        </p>
      </div>

      <div className="max-w-[900px] w-full flex flex-row flex-wrap items-center justify-center gap-2 md:gap-x-10">
        {information?.items?.map((item: any, index: number) => {
          ...
          return (
            <div
              key={`ind-${index}`}
              className="flex flex-col md:flex-row w-fit items-center px-3 gap-x-2"
            >
