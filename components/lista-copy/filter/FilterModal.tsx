...
interface FilterModalProps {
  onClose?: () => void;
  onApplyFilters: (filters: GenericFilterValues | undefined) => void;
  filters: GenericFilterValues | undefined;
  setFilters: (value:any)=>void;
  filtersObject: FiltersGeneralOptionsProps;
}

interface GenericFilterValues {
  nombre?: string;
  activo?: string;
}
...
export function GenericFilterModal({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof GenericFilterValues, string>>
  >({});
  ...
  const handleInputChange = (
    ...
    setFilters((prev: GenericFilterValues) => ({ ...prev, [name]: value }));

    // Clear errors when input changes
    if (errors[name as keyof GenericFilterValues]) {
