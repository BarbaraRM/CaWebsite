...
interface FilterModalProps {
  onClose?: () => void;
  onApplyFilters: (filters: EspecFilterValues | undefined) => void;
  filters: EspecFilterValues | undefined;
  setFilters: (value:any)=>void;
  filtersObject: FiltersGeneralOptionsProps;
}

export interface EspecFilterValues {
  nombre?: string;
  activo?: string;
}

export function EspecialidadFilterModal({
  onClose,
  onApplyFilters,
  filtersObject,
  filters,
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof EspecFilterValues, string>>
  >({});
  ...
  const handleInputChange = (
    ...
    setFilters((prev: EspecFilterValues) => ({ ...prev, [name]: value }));

    // Clear errors when input changes
    if (errors[name as keyof EspecFilterValues]) {
